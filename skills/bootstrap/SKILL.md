...
| develop-feature | 新需求开发、功能开发、完整开发流程、测试工作流、SDD/BDD/TDD 编排 |
| fix-bug | 修复 bug、故障排查、问题修复、fix bug |
| openspec | SDD、先写 spec、propose、先定方案再实现 |
| bdd | BDD、验收标准、Given-When-Then、场景登记 |
| tdd | TDD、测试驱动、红绿重构、test-driven |
| test-strategy | 测试策略、选择测试层级、怎么测 |
| verify | 用户验证、验收测试 |
| e2e-write | 编写 E2E 测试 |
| doc-sync | 文档同步、更新文档 |
